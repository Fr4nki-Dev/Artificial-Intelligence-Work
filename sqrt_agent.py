...
def sqrt_agent_abs_error(x):
    ...
    assert isinstance(x, (int, float)), "Input must be an integer or a float"
    assert x > 0, "Input must greater than 0"
...
def test_sqrt_agents():
    ...
    """
    test_values = [4, 9, 16, 25, 100, 0.25, 1.44,
                   10]  # Some test cases, including perfect squares and non-perfect squares
    ...
    for value in test_values:
        ...
        # print(f"{estimated_sqrt_abs: .6f}")
        # Check if the estimated values are close to the actual value
        # HINT: use math.isclose
        is_close_abs = math.isclose(
            estimated_sqrt_abs, actual_sqrt, abs_tol=tolerance)
        is_close_sqr = math.isclose(
            estimated_sqrt_sqr, actual_sqrt, abs_tol=tolerance)
        ...
        print(f"Estimated sqrt (Abs Error): {
              estimated_sqrt_abs:.6f}, Pass: {is_close_abs}")
        print(f"Estimated sqrt (Squared Error): {
              estimated_sqrt_sqr:.6f}, Pass: {is_close_sqr}")
